...
def fplot(f,xrange,yrange):
    ...
    step = abs(xrange[1]-xrange[0])/50
    plt.plot([x for x in np.arange(xrange[0],xrange[1]+step,step)],[f(x) for x in np.arange(xrange[0],xrange[1]+step,step)],lw=4, c='chartreuse',label='function')
    ax.set_xlim(xrange)
    ax.set_ylim(yrange)
